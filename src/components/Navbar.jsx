      ...
      <div className='w-full flex justify-between items-center max-w-7xl max-auto'>
        ...
        onClick={()=>{
         ...
         window.scrollTo(0,0);
          ...
          Faris &nbsp;
          <span className='sm:block hidden'>Rehman</span>
          </p>
        </Link>
          ...
          >
            <ul className='list-none flex justify-end items-start flex-1 flex-col gap-4'>
              {navLinks.map((nav) => (
                <li
                  key={nav.id}
                  className={`font-poppins font-medium cursor-pointer text-[16px] font-poppins font-medium cursor-pointer text-[16px]`}
                  onClick={() => {
                    setToggle(!toggle);
                    setActive(nav.title);
                  }}
